...
def map_png(xyResolution):
    ...
    img = plt.imread(path)
    img = img.sum(-1)
    img = np.where(img > 3.99, 0, 1)
    img = img.transpose()
    img = img[::-1]
    obstacleX, obstacleY = [], []
    #x scale
    img_scale = np.zeros([int(img.shape[0]/scale), int(img.shape[1]/scale)]) 
    for i in range(0, img_scale.shape[0]):
        for j in range(0, img_scale.shape[1]):
            _i = i*scale
            _j = j*scale
            value= img[_i:_i+scale, _j:_j+scale].sum()
            img_scale[i, j] = value
            if value > 0:
                obstacleX.append(i)
                obstacleY.append(j)
